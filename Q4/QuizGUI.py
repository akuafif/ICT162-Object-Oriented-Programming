...
class QuizGUI(Tk):
    ...
    def createWidgets(self) -> None:
        ...
        self.__rbtnFalse = Radiobutton(self.__tf, text='Option 2', padx = 20, variable=self.__answer, value=2)
    ...
    def submitAnswer(self) -> None:
        ...
        if str(self.__answer.get()) in '12':
            self.__btnSubmit.configure(state = DISABLED)
            self.__btnNext.configure(state = NORMAL)

            answer = True if self.__answer.get() == 1 else False

            if self.__questionBank.checkCureentAnswer(answer):
